...
function get_canvas_assignments($course_id, $headers, $source)
{
    $courseworks = [];
    ...
    foreach ($courseworks_response as $coursework) {
        array_push($courseworks, parse_coursework($coursework, $source, null, null, null));
    }
    return $courseworks;
}

function get_google_assignments($course_id, $service, $source)
{
    $courseworks = [];
    $response = $service->courses_courseWork;
    foreach ($response->listCoursesCourseWork($course_id) as $coursework) {
        array_push($courseworks, parse_coursework($coursework, $source, $service, $course_id, $coursework["id"]));
    }
    return $courseworks;
}

function get_canvas_announcements($course_id, $headers, $source)
{
    $announcements = [];
    ...
    foreach ($announcements_response as $announcement) {
        array_push($announcements, parse_announcements($announcement, $course_id, $source));
    }
    return $announcements;
}

function get_google_announcements($course_id, $service, $source)
{
    $announcements = [];
...
function parse_coursework($object, $source, $service, $google_courseId, $google_courseworkId)
{
    $coursework = [];
...
function parse_announcements($object, $course_id, $source)
{
    $announcement = [];
    if ($source === SOURCE_GOOGLE_CLASSROOM) {
